...
def get_optimizer(optimizer_type, model, args):
    ...
    if optimizer_type == "adamw":  # 修改为 adamw
        optimizer = torch.optim.AdamW(grouped_parameters, lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=args.weight_decay)
...
def get_lossFunction(lossFunc_type):
    if lossFunc_type=="L2":
        ...
        from utils.torch_utils import CombinedSmoothL1Loss
        lossFunction = CombinedSmoothL1Loss().cuda()
        lossFunction = CombinedSmoothL1WithConfidenceLoss.cuda()
    else:
